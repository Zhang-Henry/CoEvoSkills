...
EOF
export PIP_CONSTRAINT=/tmp/agentops-pip-constraints.txt
...
echo "##[group]"Run tox
echo "##[endgroup]"
# The repository's LangChain integration imports langchain-core, whose entire
# published release line requires Python >=3.8.  Replaying the historical
# unpinned `tox` command against today's package index therefore makes py37
# unsatisfiable before project tests start.  Keep the meaningful supported
# matrix deterministic instead of treating package-index drift as a code fix.
echo 'tox -e py38,py39,py310,py311,py312' > /home/github/24653510459/steps/bugswarm_3.sh
